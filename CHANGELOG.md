<!-- Generated by scripts/generate-changelog.lua from Options/ChangelogData.lua. -->
# BFInfinite changelog

## r5-alpha — 2026-08-04 00:00 GMT+8

### English (enUS)

- Party and Raid Buffs now support bounded child Buff Displays, including Healing Auras, Defensives, Externals, and user-created displays
- Buff Displays add secret-safe Icon + Duration Bar and Health Bar highlight presentations through Blizzard's native aura providers
- Individual Bags view keeps full-size icons and grows from the Combined View baseline height
- Sidebar category expansion now persists when the sidebar collapses
- Sidebar category icons across every category (equipment, consumables, trade goods, recipes, reagents, quest items, and housing) now render as larger, full-color native in-game art on plated backgrounds
- Sidebar toggle now collapses and expands the sidebar instantly, replacing the old auto-hide behavior
- The collapsed sidebar rail now shows expand/collapse chevrons next to each category icon
- The sidebar rail now sits flush with the bag window's edge

### 简体中文 (zhCN)

- 小队与团队增益现在支持有数量限制的子增益显示，包括治疗光环、减伤技能、外部减伤和用户自定义显示
- 增益显示现可通过暴雪原生光环提供器使用安全处理受限数据的“图标 + 持续时间条”和生命条高亮显示方式
- 独立背包视图保留完整大小的图标，并以合并视图的基准高度为起点向上增长
- 侧栏收起时，侧栏分类的展开状态现在会保留
- 侧栏所有分类（装备、消耗品、贸易物品、配方、材料、任务物品和房屋）的图标现在都以更大尺寸的全彩游戏内原生素材呈现，并带有底板背景
- 侧栏切换按钮现在可以立即收起或展开侧栏，取代了原有的自动隐藏方式
- 收起状态下的侧栏现在会在每个分类图标旁显示展开/收起箭头
- 侧栏现在与背包窗口边缘对齐

## r2-alpha — 2025-10-14 17:30 GMT+8

### English (enUS)

- Added class accent color support
- Added AF version check on load
- Fixed Unit Frames preset issues
- Temporary font fixes
- Updated Buffs & Debuffs options
- Updated font options
- Used complement color for logo gradient

### 简体中文 (zhCN)

- 新增职业强调色的支持
- 新增加载时的 AF 版本检查
- 修复单位框体的预设问题
- 临时修复字体相关问题
- 更新增益与减益选项
- 更新字体选项
- 为徽标渐变使用互补色

## r1-alpha — 2025-10-06 01:36 GMT+8

### English (enUS)

- Initial release

### 简体中文 (zhCN)

- 首次发布

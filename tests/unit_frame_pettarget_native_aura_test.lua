...
local function testPetTargetActivationAndConstructionOrder()
    ...
    assertTrue(
        ...
        "PetTarget native controller was not prebuilt before setup"
    )
    assertTrue(
        harness:EventIndex("legacy.create")
            < harness:EventIndex("setup"),
        "PetTarget legacy auras were not prebuilt before setup"
    )
    assertTrue(
        harness:EventIndex("setup")
    ...
    assertEqual(buffs[1], "auras",
        "PetTarget buffs legacy builder")
    ...
    assertEqual(frame.indicators.buffs.builder, "auras",
        "PetTarget legacy buffs construction")
end
...
local function testPetTargetDefaultDisabledDoesNotBuild()
    ...
    assertEqual(harness.compiles[1].unit, "pettarget",
        "disabled PetTarget compile unit")
    assertEqual(harness.controllers[1].rebuildCount, nil,
        "disabled PetTarget restricted-container rebuild")
    assertEqual(state.built, false,
        "disabled PetTarget native built state")
    assertEqual(state.pending, true,
        "disabled PetTarget deferred config state")
end
...
local function testPetTargetUnitEventsAndTicksRefreshNativeRuntime()
    ...
    assertEqual(harness.compiles[1].unit, "pettarget",
        "PetTarget runtime compile unit")
    assertEqual(controller.rebuildCount, 1,
        "PetTarget runtime construction count")
    ...
    assertEqual(controller.refreshCount, initialRefreshCount + 1,
        "PetTarget periodic native refresh count")
    assertEqual(#harness.compiles, 1,
        "PetTarget tick recompiled stable runtime")
    assertEqual(controller.rebuildCount, 1,
        "PetTarget tick rebuilt stable runtime")
    assertEqual(controller.setUnitCount or 0, initialSetUnitCount,
        "PetTarget tick retargeted stable runtime")

    root.scripts.OnEvent(root, "UNIT_TARGET", "pet")
    assertEqual(controller.refreshCount, initialRefreshCount + 2,
        "PetTarget UNIT_TARGET native refresh count")
    assertEqual(#harness.compiles, 1,
        "PetTarget UNIT_TARGET recompiled stable runtime")
    assertEqual(controller.rebuildCount, 1,
        "PetTarget UNIT_TARGET rebuilt stable runtime")
    assertEqual(controller.setUnitCount or 0, initialSetUnitCount,
        "PetTarget UNIT_TARGET retargeted stable runtime")
    ...
    assertEqual(native.Update, nativeUpdate,
        "PetTarget native Update method identity")

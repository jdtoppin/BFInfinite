...
local function ReconcileViewer()
    ...
    local geometryChanged = state.geometryChanged or false
    state.geometryChanged = nil
    return NextResult("reconcileFailures"), geometryChanged
end
...
function PresentationMethods.AdvanceTargetTransition(
    ...
    state.advanceCalls = (state.advanceCalls or 0) + 1
    state.lastTransitionComplete = complete
    state.lastGeometryChanged = geometryChanged
    state.lastTransitionElapsed = elapsed

    local transition = presentationController.targetTransition
    if transition and transition.active then
        if state.finishTargetTransition then
            state.finishTargetTransition = nil
...
local callsBeforeTargetTransition = controller.state.reconcileCalls
controller.controller.targetTransition = {active = true}
controller.state.reconcileFailures = 100
controller.state.geometryChanged = true
controller.queue()
controller.run(0.01)
assertEqual(controller.state.lastGeometryChanged, true,
    "reconcile geometry result reaches target transition controller")
for _ = 1, 8 do
    controller.run(0.01)
end
assertEqual(controller.state.reconcileCalls, callsBeforeTargetTransition + 9,
    "active target transition bypasses ordinary debounce")
assertEqual(controller.state.lastGeometryChanged, false,
    "later stable geometry result reaches target transition controller")
assertEqual(controller.controller.retryPassesRemaining, 7,
    "active target transition does not consume retry budget")
assertEqual(type(controller.controller.scripts.OnUpdate), "function",
    "active target transition stays awake past ordinary retry exhaustion")

controller.state.reconcileFailures = 0
controller.state.geometryChanged = true
controller.state.finishTargetTransition = true
controller.run(0.01)
assertEqual(controller.state.lastGeometryChanged, true,
    "completion pass forwards geometry changes")
assertEqual(controller.controller.scripts.OnUpdate, nil,
    "completed target transition sleeps after successful reconciliation")

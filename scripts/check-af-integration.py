...
def parse_lock(path: Path) -> FrameworkLock:
    ...
    if values["repository"] != EXPECTED_REPOSITORY:
        raise IntegrationError(
            ...
            f"got {values['repository']!r}"
        )
    if REVISION_PATTERN.fullmatch(values["revision"]) is None:
        raise IntegrationError(
            f"{path}: revision must be a full lowercase 40-character commit SHA"
        )
...
def toc_value(path: Path, field: str) -> str:
    ...
    if len(matches) != 1:
        raise IntegrationError(
            f"{path}: expected exactly one ## {field} field, found {len(matches)}"
        )
    return matches[0]
...
def one_numeric_assignment(path: Path, pattern: re.Pattern[str], label: str) -> int:
    ...
    if len(matches) != 1:
        raise IntegrationError(
            f"{path}: expected exactly one {label} assignment, found {len(matches)}"
        )
    return matches[0]


def lua_executable_source(source: str) -> str:
    """Mask Lua comments and string literals while preserving line boundaries."""
    ...
    def long_bracket(start: int) -> tuple[int, str] | None:
        ...
        if cursor >= length or source[cursor] != "[":
            return None
        equals = source[start + 1 : cursor]
        return cursor + 1, f"]{equals}]"

    index = 0
    while index < length:
        if source.startswith("--", index):
            opening = long_bracket(index + 2)
            if opening is None:
                end = source.find("\n", index + 2)
                if end == -1:
                    end = length
                ...
                end = length if close_at == -1 else close_at + len(closing)
            mask(index, end)
            index = end
            continue
...
def validate_source_contract(
    ...
    if actual_revision != lock.revision:
        raise IntegrationError(
            f"AbstractFramework checkout is {actual_revision}, "
            f"expected locked revision {lock.revision}"
        )
    ...
    assignment = re.compile(
        r"^\s*BFI\.requiredAFVersion\s*=\s*([0-9]+)\s*(?:--.*)?$"
    )
    core_constant = re.compile(
        r"^\s*local\s+REQUIRED_AF_VERSION\s*=\s*([0-9]+)\s*(?:--.*)?$"
    )
    ...
    core_source = lua_executable_source(
        (bfi_root / "Core.lua").read_text(encoding="utf-8")
    )
    if re.search(
        r"^[ \t]*AF\.RequireVersion[ \t]*\([ \t]*"
        r"REQUIRED_AF_VERSION[ \t]*\)[ \t]*;?[ \t]*$",
        core_source,
        re.MULTILINE,
    ) is None:
        raise IntegrationError(
            "Core.lua must enforce AF.RequireVersion(REQUIRED_AF_VERSION)"
        )

    af_version = toc_value(af_root / "AbstractFramework.toc", "Version")
    if af_version != lock.version:
        raise IntegrationError(
            f"AbstractFramework.toc declares {af_version!r}, "
            f"expected locked version {lock.version!r}"
        )
...
def git_head(path: Path) -> str:
    ...
    if result.returncode != 0:
        ...
        raise IntegrationError(
            f"cannot inspect AbstractFramework checkout at {path}: {detail}"
        )
...
def main() -> int:
    ...
    try:
        ...
        if args.export_github_output is not None:
            ...
            print(
                "AbstractFramework lock passed: "
                f"{lock.repository}@{lock.revision} ({lock.version})"
            )
            return 0
    ...
    print(
        "AbstractFramework integration passed: "
        f"{lock.repository}@{lock.revision} ({lock.version})"
    )
    return 0
